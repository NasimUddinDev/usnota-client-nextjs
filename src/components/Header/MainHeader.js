...
import { AiOutlineLogin,AiOutlineUnorderedList,AiOutlineSetting } from "react-icons/ai";
...
export default function MainHeader() {
  ...
  useEffect(() => {
    window.addEventListener("click", (e) => {
      if (!e.target.closest(".accountDropdownBtn") && !e.target.closest(".userInfo")) {
              ...
              >
...

                {
                  ...
                  <div className="relative">
                    <button 
                      ...
                      className="accountDropdownBtn flex items-center gap-1 hover:text-primary duration-200" >
                      <h1 className="font-medium">My Account</h1>
                      {
                        accountDropdown ? <MdKeyboardArrowUp className="text-xl" /> : <MdKeyboardArrowDown className="text-xl" />
                      }
                    </button>
                        ...
                        <ul className="absolute min-w-52 w-max bg-base-100 right-0 top-[41px] shadow-lg rounded z-50 overflow-hidden text-neutral">
                          <li className="border-b p-2 userInfo">
                            <div className="flex gap-2 items-center">
                              ...
                              <div>
                                <h2>Nasim Uddin</h2>
                                <p className="text-sm text-neutral/80">nasimuddin@gmail.com</p>
                              </div>
                            </div>
                          </li>
                          ...
                          <li>
                            <Link 
                              href="/account/orders" 
                              className="border-b px-3 py-1.5 flex items-center gap-1 hover:bg-gray-200 hover:text-primary duration-300">
                              <AiOutlineUnorderedList className="text-base" /> My Order
                            </Link>
                          </li>
                ...
                :
                  <div>
                    ...
                    <SignUpInModal setFormToggle={setFormToggle} formToggle={formToggle} />
                  </div>
                }
            </div>
          </div>
